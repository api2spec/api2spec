...
// koaTestCode is a comprehensive test fixture for Koa route extraction.
const koaTestCode = `
import Koa from 'koa'
...
router.post('/users', validate(CreateUserSchema), (ctx) => { ctx.body = {} })
...
// koaRouterPrefixCode tests router with prefix option.
const koaRouterPrefixCode = `
import Koa from 'koa'
import Router from '@koa/router'
...
api.get('/users', (ctx) => { ctx.body = [] })
api.get('/users/:id', (ctx) => { ctx.body = {} })
api.post('/users', (ctx) => { ctx.body = {} })
...
// koaNestedRouterCode tests nested routers.
const koaNestedRouterCode = `
...
userRouter.post('/', (ctx) => { ctx.body = {} })

const postRouter = new Router()
postRouter.get('/', (ctx) => { ctx.body = [] })
postRouter.get('/:id', (ctx) => { ctx.body = {} })
...
// koaAllMethodsCode tests all HTTP methods.
const koaAllMethodsCode = `
import Router from 'koa-router'

const router = new Router()

router.get('/test', (ctx) => { ctx.body = 'get' })
router.post('/test', (ctx) => { ctx.body = 'post' })
router.put('/test', (ctx) => { ctx.body = 'put' })
router.delete('/test', (ctx) => { ctx.body = 'delete' })
router.patch('/test', (ctx) => { ctx.body = 'patch' })
router.head('/test', (ctx) => { ctx.body = 'head' })
router.options('/test', (ctx) => { ctx.body = 'options' })

export default router
`

// koaAtRouterCode tests @koa/router import.
const koaAtRouterCode = `
import Router from '@koa/router'

const router = new Router()

router.get('/products', (ctx) => { ctx.body = [] })
router.get('/products/:id', (ctx) => { ctx.body = {} })
...
func TestPlugin_Info(t *testing.T) {
	...
	assert.Equal(t, "koa", info.Name)
	assert.Equal(t, "1.0.0", info.Version)
	assert.NotEmpty(t, info.Description)
	assert.Contains(t, info.SupportedFrameworks, "koa")
	assert.Contains(t, info.SupportedFrameworks, "koa-router")
	assert.Contains(t, info.SupportedFrameworks, "@koa/router")
}

func TestPlugin_Detect_WithKoa(t *testing.T) {
	dir := t.TempDir()
	packageJSON := `{
  "name": "test-app",
  "dependencies": {
...
func TestPlugin_Detect_WithKoaDevDep(t *testing.T) {
	dir := t.TempDir()
	packageJSON := `{
  "name": "test-app",
  "devDependencies": {
...
func TestPlugin_Detect_WithoutKoa(t *testing.T) {
	dir := t.TempDir()
	packageJSON := `{
  "name": "test-app",
  "dependencies": {
...
func TestPlugin_ExtractRoutes_BasicRoutes(t *testing.T) {
	...
	// Should have request body from validate middleware
	...
	files := []scanner.SourceFile{
		{
			Path:     "app.ts",
			Language: "typescript",
			Content:  []byte(koaRouterPrefixCode),
		},
	}
...
func TestPlugin_ExtractRoutes_NestedRouters(t *testing.T) {
	p := New()
	...
	files := []scanner.SourceFile{
		{
			Path:     "app.js",
			Language: "javascript",
			Content:  []byte(koaNestedRouterCode),
		},
	}
	...
	// Should extract routes from nested routers
	// Note: Nested router prefix resolution may need additional logic
	...
	files := []scanner.SourceFile{
		{
			Path:     "app.ts",
			Language: "typescript",
			Content:  []byte(koaAllMethodsCode),
		},
	}
...
func TestPlugin_ExtractRoutes_AtKoaRouter(t *testing.T) {
	p := New()
...
func TestPlugin_ExtractRoutes_IgnoresNonKoa(t *testing.T) {
	p := New()

	// File that doesn't import Koa
	code := `
