...
// Package gin provides a plugin for extracting routes from Gin framework applications.
package gin
...
// ginImportPaths are the known import paths for Gin framework.
var ginImportPaths = []string{
	"github.com/gin-gonic/gin",
}

// httpMethods are the HTTP methods supported by Gin.
...
// httpMethodFuncs maps Gin method names to HTTP methods.
var httpMethodFuncs = map[string]string{
	...
	"HEAD":    "HEAD",
	"OPTIONS": "OPTIONS",
	"Any":     "ANY", // Special case: matches all methods
}

// Plugin implements the FrameworkPlugin interface for Gin framework.
type Plugin struct {
	goParser        *parser.GoParser
	schemaExtractor *schema.GoSchemaExtractor
}

// New creates a new Gin plugin instance.
func New() *Plugin {
	return &Plugin{
...
func (p *Plugin) Info() plugins.PluginInfo {
	return plugins.PluginInfo{
		Name:        "gin",
		Version:     "1.0.0",
		Description: "Extracts routes from Gin framework applications",
...
// Detect checks if Gin is used in the project by looking at go.mod.
func (p *Plugin) Detect(projectRoot string) (bool, error) {
	...
	for scanr.Scan() {
		line := scanr.Text()
		for _, importPath := range ginImportPaths {
...
// ExtractRoutes parses source files and extracts Gin route definitions.
...
func (p *Plugin) extractRoutesFromFile(file scanner.SourceFile) ([]types.Route, error) {
	...
	// Check if this file imports Gin
	if !p.hasGinImport(pf) {
		return nil, nil
	}
...
func (p *Plugin) extractRoutesFromFunc(funcDecl *ast.FuncDecl, ctx *extractionContext) []types.Route {
	...
	ast.Inspect(funcDecl.Body, func(n ast.Node) bool {
		...
		// Check for Handle() calls - e.g., r.Handle("GET", "/path", handler)
		if methodName == "Handle" {
			route := p.extractRouteFromHandle(callExpr, ctx)
			if route != nil {
...
func (p *Plugin) handleRouteGroup(callExpr *ast.CallExpr, ctx *extractionContext) []types.Route {
	...
	// Find the parent of this call to look for chained method calls
	// In Gin, groups are typically used like:
	// api := r.Group("/api")
	...
	// For now, we need to find assignments and trace the variable usage
	// This is a simplified approach that handles inline chains

	// Check for chained calls on the same expression
	// e.g., r.Group("/api").GET("/users", handler)
...
func (p *Plugin) extractRouteFromCall(callExpr *ast.CallExpr, method string, ctx *extractionContext) *types.Route {
	...
	// Convert Gin :param syntax to OpenAPI {param} syntax
	fullPath = convertGinPathParams(fullPath)
	...
	httpMethod := httpMethodFuncs[method]
	if httpMethod == "" {
...
// extractRouteFromHandle extracts a Route from r.Handle("GET", "/path", handler).
func (p *Plugin) extractRouteFromHandle(callExpr *ast.CallExpr, ctx *extractionContext) *types.Route {
	if len(callExpr.Args) < 2 {
		return nil
	}
...
// hasGinImport checks if the file imports Gin.
func (p *Plugin) hasGinImport(pf *parser.ParsedFile) bool {
	for _, importPath := range ginImportPaths {
...
// convertGinPathParams converts Gin :param syntax to OpenAPI {param} syntax.
func convertGinPathParams(path string) string {
	// Convert :param to {param}
	path = regexp.MustCompile(`:(\w+)`).ReplaceAllString(path, "{$1}")

	// Convert *param to {param} (catch-all in Gin)
	path = regexp.MustCompile(`\*(\w+)`).ReplaceAllString(path, "{$1}")
...
// Register registers the Gin plugin with the global registry.
func Register() {
